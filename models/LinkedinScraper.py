...
from controllers.alertDialogController import Alerts, AlertType, IconType, AlertButton
...
class LinkedinScraper():
    ...
    def __init__(self, webview,jobTitle,country,location,includes,excludes):
        super().__init__()
    ...
    def start(self,fn_callback):
        '''
            Start the Scraper
        '''
        if self.build_query():
            fn_callback.emit(lambda: self.webview.load(QUrl(self.urls.pop(0))))
    ...
    def scrapeResult(self, html):
        ...
        if recaptcha:
            ...
            if self.webview.isVisible():
                self.webview.hide()
            if pnnext:
                self.webview.load(QUrl('https://www.google.com' + pnnext.attrs['href']))
            else:
                if len(self.urls)!=0:
                    self.webview.load(QUrl(self.urls.pop(0)))
                else:
                    print('************** all urls finished **************')
    ...
    def build_query(self):
        ...
        if not e:
            ...
            return False
        self.urls = [
            f'https://www.google.com/search?q=site:{e}.linkedin.com "{self.jobTitle}" "{self.location}" "{self.includes}" intext:("@gmail.com" | "@hotmail.com" | "@live.com" | "@outlook.com" | "@yahoo.com") -inurl:"dir/" -intext:"{self.excludes}"',
            ...
            f'https://www.google.com/search?q=site:"www.linkedin.com" "{self.country}" "{self.jobTitle}" "{self.location}" "{self.includes}" intext:("@gmail.com" | "@hotmail.com" | "@live.com" | "@outlook.com" | "@yahoo.com") -inurl:"dir/" -intext:"{self.excludes}"',
    ...
    def getCountryCode(self,country):
        for key,value in LinkedinScraper.countries.items():
